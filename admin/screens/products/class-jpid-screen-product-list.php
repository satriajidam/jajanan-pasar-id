...
/**
 * Manage jpid_product post list screen.
 *
 * @since      1.0.0
...
class JPID_Admin_Product_List {
  ...
  public function display_column_data( $column_name ) {
    ...
    $current_product = new JPID_Product( $current_post->ID );

    switch ( $column_name ) {
      case 'name':
        echo '
          <strong>
            <a href="' . esc_url( $current_product->get_editlink() ) . '" class="row-title">' . esc_html( $current_product->get_name() ) . '</a>
          </strong>
        ';
  ...
  public function set_quick_edit( $column_name ) {
    ...
    include_once JPID_PLUGIN_DIR . 'admin/screens/products/views/html-jpid-quick-edit-product-list.php';
  }
